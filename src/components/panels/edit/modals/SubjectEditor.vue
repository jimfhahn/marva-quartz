...
export default {
  ...
  methods: {
    ...
    parseComplexSubject: async function (uri) {
      ...
      let url = uri
      // Penn: Use production id.loc.gov only (no pre-prod access)
    ...
    */
    ...
    buildPickLookup: function () {
      // Safety check: ensure searchResults exists and has expected structure
      if (!this.searchResults) {
        console.warn('buildPickLookup: searchResults is null/undefined')
        return
      }

      // Ensure all arrays exist, default to empty arrays if not
      const subjectsComplex = this.searchResults.subjectsComplex || []
      const subjectsChildrenComplex = this.searchResults.subjectsChildrenComplex || []
      const subjectsSimple = this.searchResults.subjectsSimple || []
      const subjectsChildren = this.searchResults.subjectsChildren || []
      const names = this.searchResults.names || []
      const exact = this.searchResults.exact || []

      for (let x in subjectsComplex) {
        this.pickLookup[x] = subjectsComplex[x]
      }

      for (let x in subjectsChildrenComplex) {
        this.pickLookup[x] = subjectsChildrenComplex[x]
      }

      for (let x in subjectsSimple) {
        this.pickLookup[parseInt(x) + parseInt(subjectsComplex.length)] = subjectsSimple[x]
      }

      for (let x in subjectsChildren) {
        this.pickLookup[parseInt(x) + parseInt(subjectsChildrenComplex.length)] = subjectsChildren[x]
      }

      for (let x in names) {
        this.pickLookup[(names.length - x) * -1] = names[x]
      }

      for (let x in exact) {
        this.pickLookup[(names.length - x) * -1 - 2] = exact[x]
      }

      console.log('buildPickLookup completed. Total entries:', Object.keys(this.pickLookup).length)
    },
    ...
    searchApis: debounce(async (searchString, searchStringFull, that) => {
      ...
      if (searchString.length > 2){
        ...
        // Not enough characters - clear loading state and return early
        window.clearInterval(ti)
        window.clearTimeout(tiBackup)
        that.activeSearch = false
        console.log('Search requires at least 3 characters, got:', searchString.length)
        return
      }
      // that.searchResults = await utilsNetwork.subjectSearch(searchString, searchStringFull, that.searchMode)

      // Safety check: ensure searchResults is valid before processing
      if (!that.searchResults) {
        console.warn('searchApis: Search returned null/undefined results')
        window.clearInterval(ti)
        window.clearTimeout(tiBackup)
        that.activeSearch = false
        return
      ...
      if ((that.searchResults.hierarchicalGeographic?.length || 0) > 0 && (that.searchResults.subjectsComplex?.length || 0) == 0) {
        that.searchResults.subjectsComplex = that.searchResults.hierarchicalGeographic
      }

      that.pickLookup = {}

      // Calculate pickPosition safely - default to 0 if no results
      const totalSubjects = (that.searchResults.subjectsSimple?.length || 0) + (that.searchResults.subjectsComplex?.length || 0)
      that.pickPostion = totalSubjects > 0 ? totalSubjects - 1 : 0
      ...
      if (Object.keys(that.pickLookup).length > 0 && that.pickLookup[that.pickPostion] && !that.pickLookup[that.pickPostion].literal) {
      ...
      } else if (Object.keys(that.pickLookup).length === 0) {
        console.log('No search results to display in pickLookup')
      }
    ...
    getContext: async function () {
      // Safety check: ensure pickLookup entry exists
      if (!this.pickLookup[this.pickPostion]) {
        console.warn('getContext: pickLookup entry does not exist for position:', this.pickPostion)
        return false
      }
      
      const entry = this.pickLookup[this.pickPostion]
      
      if (entry.literal) {
        this.contextData = entry
        return false
      }

      // Ensure extra data exists with safe defaults
      if (!entry.extra || typeof entry.extra !== 'object') {
        console.warn('getContext: Missing extra data, creating defaults for entry:', entry)
        entry.extra = {
          rdftypes: ['Topic'],
          collections: [],
          gacs: [],
      ...
      if (!Array.isArray(extra.rdftypes)) extra.rdftypes = ['Topic']
      if (!Array.isArray(extra.collections)) extra.collections = []
      if (!Array.isArray(extra.gacs)) extra.gacs = []
      if (!Array.isArray(extra.marcKeys)) extra.marcKeys = []

      this.contextData = extra

      if (entry.uri) {
        // Pull information into contextData from 1 level up
        this.contextData.literal = false
        this.contextData.title = entry.label
        this.contextData.uri = entry.uri
        
        if (extra.marcKeys && extra.marcKeys.length > 0) {
          entry.marcKey = extra.marcKeys[0]
        } else if (extra.marcKey) {
          entry.marcKey = extra.marcKey
        }

        // Safely get types array with default
        const types = extra.rdftypes || ['Topic']

        // Setting the type in contextData - now safe to use types.includes()
        ...
        // Check if it's a Jurisdiction, and overwrite - now safe to use collections.includes()
        const collections = extra.collections || []
        if (collections.includes("http://id.loc.gov/authorities/names/collection_Jurisdictions")) {
          this.contextData.type = "bf:Jurisdiction"
          this.contextData.typeFull = "http://id.loc.gov/ontologies/bibframe/Jurisdiction"
        }

        this.contextData.gacs = extra.gacs || []
    ...
    selectContext: async function (pickPostion, update = true) {
      ...
      // Safety check: ensure pickLookup entry exists
      if (!this.pickLookup[this.pickPostion]) {
        console.warn('selectContext: pickLookup entry does not exist for position:', this.pickPostion)
        return false
      }
